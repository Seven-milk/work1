...
# extract variable(given region by coord) from .nc4 file - writing-while-reading to avoid limitation in memory
import numpy as np
...
def extract_nc(path, coord_path, variable_name, r, precision=3, coordsave=False):
    ...
    for i in range(len(result)):
        ...
        for j in range(len(coord)):
            variable.append(str(float(f.variables[variable_name][0, lat_index[j], lon_index[j]])))

...
for station in stations:
    result = pd.DataFrame(np.full((len(pd_index), 11), fill_value=-9999), index=pd_index)
    stms = [os.path.join(network_china, station, d) for d in os.listdir(os.path.join(network_china, station)) if d[-4:] == ".stm"]
